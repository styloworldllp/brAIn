...
from services.data_loader import (
    load_csv, load_excel, load_from_db, test_db_connection,
    load_google_sheet_public, load_google_sheet_service_account,
...
def list_datasets(db: Session = Depends(get_db)):
    ...
    return [_serialise_dataset(d) for d in datasets]
...
async def upload_file(
    ...
    try:
        if ext == ".csv":
            df = load_csv(upload_path)
        else:
            df = load_excel(upload_path)
...
class DBConnectRequest(BaseModel):
    ...
    username: str
    password: str
    table_or_query: str   # table name or SELECT query
...
class SheetsConnectRequest(BaseModel):
    name: str
    sheet_url: str
    service_account_json: Optional[str] = None   # paste the JSON content (optional)
...
def connect_sheets(req: SheetsConnectRequest, db: Session = Depends(get_db)):
    try:
        if req.service_account_json:
            df = load_google_sheet_service_account(req.sheet_url, req.service_account_json)
        else:
            df = load_google_sheet_public(req.sheet_url)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc))
