...
def _build_data_setup(dataset: dict) -> str:
    ...
    if conn_str and table_query:
        ...
        safe_conn = conn_str.replace('"', '\\"')
        return (
            f'from sqlalchemy import create_engine, text as _text\n'
...
def _build_extra_datasets_setup(extra_datasets: list) -> str:
    ...
    for ds in extra_datasets:
        ...
        if table_query == "__live__" and conn_str:
            ...
            lines.append(f'from sqlalchemy import create_engine, text as _text')
...
def execute_python(code: str, dataset: dict, extra_datasets: list | None = None) -> dict:
